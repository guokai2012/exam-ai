...
- `vo`：前端返回视图对象。
- `dto`：前端传入参数对象。
- `common/base`：基类、通用枚举；后端实体公共字段必须统一放在 `BaseEntity`。
- `common/result`：统一返回值。
...
- 所有 Controller 类必须显式使用类级 `@RequestMapping("路径")` 设置接口基础路径，类级路径至少到 `/api/{模块名}` 或 `/api/{业务资源名}`，禁止只设置为 `/api`，也禁止只在方法级 Mapping 上拼完整路径；当同一 Controller 覆盖多个资源根路径时必须拆分 Controller。
- 接口路径统一使用 `/api/xxx` 前缀。
...
- 所有表设计中的依赖字段（例如 `user_id`、`role_id`、`document_id`、`parent_id`、`permission_id` 等）只能作为普通字段或普通索引，不允许定义 `FOREIGN KEY`、`REFERENCES`、级联删除或级联更新约束。
...
- 所有表格列表中的行级操作按钮必须统一放在第一列，列标题为“操作”，并使用 Element Plus `fixed="left"` 固定该列，便于横向滚动时始终可操作。
- 表格“操作”列中的可见行级操作控件最多 2 个，普通按钮和下拉菜单触发按钮都计入数量；超过 2 个操作时必须保留 1 个最高频主操作，并将其余操作收纳到 Element Plus 下拉菜单中，避免操作列过宽。
- 新增和编辑操作应优先共用一个弹窗。
...
### 3.7 菜单与路由治理规范
- 核心内置菜单结构、页面 `path`、权限码、排序初始值、图标和 `api_path` 必须由开发人员通过可重复执行的数据库迁移脚本维护；后台允许管理员新增、编辑和删除扩展菜单。
- 菜单表禁止使用 `component` 字段，前端不得要求管理员维护组件标识；前端路由组件由代码路由表负责绑定。
- `path = NULL` 的菜单是纯分组菜单，只能展开子菜单，不允许跳转，也禁止设置 `api_path`。
- 叶子菜单必须配置非空页面 `path`；页面主资源基础 API 路径优先从当前菜单的 `api_path` 读取，未配置时才允许使用模块默认路径兜底。
- 管理员后台允许新增、编辑和删除菜单，但必须遵守 `path = NULL` 分组规则、禁止维护组件标识，并且存在子菜单的菜单不允许删除。
- `api_path` 只能从后端扫描 Controller `@RequestMapping` 得到的 API 根路径下拉选项中选择，中文名称优先使用 Controller `@Tag(name)`。
- 前端菜单扫描同步只能由 ADMIN 在菜单管理页手动触发；同步前必须先获取菜单扫描专用短时 Token，Token 默认有效 30 秒、最长 180 秒，并且同一用户会话 5 分钟内最多获取 1 次。
- 菜单扫描同步接口必须同时校验 JWT 登录态、ADMIN 角色、`admin:menu:scan` 权限和请求头 `X-Menu-Scan-Token`，临时 Token 校验成功后必须立即作废，禁止重复使用。
- 菜单扫描同步必须以当前前端 Vue Router 扫描结果作为权威数据源全量覆盖 `sys_menu`，包括覆盖已有菜单字段、创建缺失菜单、逻辑删除未扫描到的旧菜单。
- AI 生成前端页面、新路由或调整页面入口时，必须同步生成或更新菜单迁移脚本，确保菜单闭环随代码一起交付。
...
- 数据库迁移脚本禁止创建外键约束；涉及依赖字段时只能添加普通列、普通索引或唯一索引，关联一致性必须由业务代码保证。
