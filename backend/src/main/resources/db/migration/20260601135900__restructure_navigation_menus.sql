...
WHERE NOT EXISTS (
    SELECT 1 FROM sys_menu WHERE path = '/system-management'
);

INSERT INTO sys_menu (menu_name, path, component, icon, sort_order, status, permission_code)
SELECT '题库管理', '/question-management', 'MenuGroup', 'Collection', 20, 1, NULL
WHERE NOT EXISTS (
    SELECT 1 FROM sys_menu WHERE path = '/question-management'
);

UPDATE sys_menu
SET parent_id = (SELECT id FROM (SELECT id FROM sys_menu WHERE path = '/system-management' LIMIT 1) system_group),
...
UPDATE sys_menu
SET parent_id = (SELECT id FROM (SELECT id FROM sys_menu WHERE path = '/question-management' LIMIT 1) question_group),
    menu_name = '可用题',
...
WHERE g.path = '/question-management'
  AND NOT EXISTS (
      SELECT 1 FROM sys_menu WHERE path = '/questions/pending-confirm'
  );
