...
## 功能目标
菜单结构由开发人员通过迁移脚本维护，后台菜单管理只允许管理员调整菜单名称、图标、排序、启用状态和叶子菜单的页面主资源 `api_path`。`sys_menu` 不保存前端组件标识，前端路由组件由代码路由表绑定。

## 参与角色
- 管理员：维护菜单展示字段和叶子菜单 `api_path`。
...
5. 管理员编辑菜单名称、图标、排序、状态；叶子菜单可从下拉中选择 `api_path`，分组菜单不能设置 `api_path`。
6. 系统启动、后台定时任务或管理员点击“扫描权限”时，从 Controller 注解全量同步权限。
7. 菜单 `permission_code` 绑定扫描生成的动作权限码，例如 `admin:menu:list`、`document:list`。
8. 登录用户进入后台布局时，前端调用 `GET /api/menus/me`。
9. 后端根据用户权限过滤启用菜单，并移除没有可见子菜单的空分组。
10. 前端渲染左侧菜单；分组只展开不跳转，叶子菜单按 `path` 跳转，页面请求优先使用菜单 `api_path`。
...
- 接口：`GET /api/menus/me`、`GET /api/admin/menus`、`PUT /api/admin/menus/{id}`、`GET /api/admin/menus/api-path-options`、`POST /api/admin/permissions/scan`。
