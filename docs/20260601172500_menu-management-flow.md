...
9. 后端校验 JWT、ADMIN 角色、`admin:menu:scan` 权限和短时 Token，Token 校验成功后立即作废，并只补齐缺失菜单或空白开发字段。
10. 系统启动、后台定时任务或管理员点击“扫描权限”时，从 Controller 注解全量同步权限。
11. 权限扫描以 Controller 类级 `@RequestMapping` 根路径反查菜单 `api_path`，命中后使用菜单名称作为权限分组名称；同一 `api_path` 绑定多个菜单时使用 `/` 合并菜单名称，未命中时回退到 `@Tag(name)`。
12. 菜单 `permission_code` 绑定扫描生成的动作权限码，例如 `admin:menu:list`、`document:list`。
13. 登录用户进入后台布局时，前端调用 `GET /api/menus/me`。
14. 后端根据用户权限过滤启用菜单，并移除没有可见子菜单的空分组。
15. 前端渲染左侧菜单；分组只展开不跳转，叶子菜单按 `path` 跳转，页面请求优先使用菜单 `api_path`。
