...
## 功能目标
老师上传 PDF 文档后，系统先在后台生成页级分片；前端按真实页数展示分片进度。分片完成并进入 `PAGE_READY` 后，老师可手动触发 AI 页级解析，解析结果最终入库为待确认题目。

## 参与角色
- 老师：上传 PDF、查看真实分片进度、触发 AI 解析、查看解析结果。
- 系统：保存文件、读取总页数、渲染 PDF 页图片、调用 AI、处理 raw_json 并写入题库。
...
8. 后端逐页调用 AI 识别并保存 raw_json；全部成功后文档进入 `AI_PARSE_COMPLETE`。
9. raw_json 后处理定时任务将题目写入题库，状态为 `PARSE_PENDING_CONFIRM`，文档进入 `PENDING_CONFIRMATION`。

## 异常流程
- 分片失败：文档进入 `PAGE_RENDER_FAILED`，前端显示失败提示并禁用 AI 解析按钮。
- 单页 AI 解析失败：文档进入 `AI_PARSE_FAILED_REVIEW`，老师可重试失败页或确认跳过。
- raw_json 后处理失败：文档进入 `PARSE_FAILED`，系统记录错误并发送站内通知。
