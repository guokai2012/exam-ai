...
- 历史表残留旧审计字段：迁移脚本先将 `uploaded_by`、`created_by`、`updated_by`、`created_at`、`updated_at` 回填到 BaseEntity 字段，再删除旧字段，避免新代码插入时被旧非空列阻断。
...
- 前端接口返回统一使用 VO 字段，不直接暴露数据库字段名；例如文档上传人由 `createId` 表示。
- 后端实体字段从历史 `uploaded_by/created_at/updated_at/created_by/updated_by` 迁移到 `create_id/create_time/update_id/update_time`。
