...
## 参与角色
- 管理员：扫描权限、查看权限树、处理缺少接口摘要告警、为角色分配权限。
- 系统：启动和定时扫描 Controller 权限、全量同步权限表、清理过期授权关系、发送缺少接口摘要站内告警。
...
5. 后端按 Controller 类级 `@RequestMapping` 根路径反查菜单 `api_path` 生成分组名称，未命中菜单时使用 `@Tag(name)` 兜底。
...
flowchart TD
    ...
    G -- "否" --> H["跳过接口"]
    G -- "是" --> I["读取菜单分组和 @Operation.summary"]
    I --> J["新增或更新扫描权限"]
    J --> K["删除过期权限并清理角色关系"]
    I --> L{"缺 summary"}
    L -- "是" --> M["通知 ADMIN 用户处理"]
