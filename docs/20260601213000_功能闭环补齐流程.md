...
## 功能目标
补齐认证续期、注册、分页、题目分类、题目详情、文档详情、权限扫描、菜单 API 路径绑定和主动修改密码入口，确保后端已有能力在前端具备对应页面、入口、交互和异常处理。
...
8. 管理员维护菜单时可新增、编辑、删除菜单，但不能维护组件标识，分组菜单不能设置 API 路径。
9. 登录用户可从账号菜单进入修改密码页。
...
- 分组菜单设置 API 路径时后端拒绝保存，叶子菜单 API 路径只能从 Controller 扫描选项中选择。
...
- `AdminMenusPage.vue` 对应菜单维护，支持新增、编辑、删除，并约束分组菜单不能设置 API 路径。
