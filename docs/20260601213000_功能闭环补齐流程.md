...
## 功能目标
补齐认证续期、注册、分页、题目分类、题目详情、文档详情、权限新增、菜单组件约束和主动修改密码入口，确保后端已有能力在前端具备对应页面、入口、交互和异常处理。
...
7. 管理员在权限管理页新增动作权限，角色授权树可继续分配该权限。
...
- 分页、详情、分类新增、权限新增失败时使用 Element Plus 消息提示，不破坏当前页面状态。
...
- `AdminPermissionsPage.vue` 对应权限新增、扫描、编辑和删除。
